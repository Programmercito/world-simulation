...
export class SimulationService {
  ...
  public getStats() {
    ...
    return {
      population: this.world.individuals.length,
      food: this.world.foodSources.length,
      seekingMate: seeking,
  ...
  public addFood() {
    this.world.foodSources.push(
      this.foodFactory.createRandomFood(this.world.width, this.world.height),
    );
  ...
  public removeFoodBatch(quantity: number) {
    ...
    this.soundService.play('foodSpawn');
    this.addEvent(`🗑️ -${itemsToRemove} food removed`);

    console.log(`Removed ${itemsToRemove} food items from the world (requested: ${quantity})`);
  }

  public placeFoodAt(x: number, y: number) {
    const clampedX = Math.max(0, Math.min(this.world.width, x));
    const clampedY = Math.max(0, Math.min(this.world.height, y));

    const food = this.foodFactory.createRandomFood(this.world.width, this.world.height);
    food.x = clampedX;
    food.y = clampedY;
    this.world.foodSources.push(food);

    this.soundService.play('foodSpawn');
    this.addEvent(`🎯 Food placed at (${Math.round(clampedX)}, ${Math.round(clampedY)})`);
  }
  ...
  public getFoodSources() {
    return [...this.world.foodSources];
  }

  public getCivilizations() {
    return this.world.civilizations.map((civ) => {
      const population = this.world.individuals.filter(
        (ind) => ind.isAlive && ind.civilizationId === civ.id,
      ).length;
  ...
  constructor(
    ...
    if (foodSpawnIntervalSeconds && foodSpawnIntervalSeconds > 0) {
      this.ticksPerFoodSpawn = Math.floor(
        (foodSpawnIntervalSeconds * 1000) / this.world.cycleDurationMs,
      );
    }
  }
  ...
  public setFoodSpawnInterval(intervalSeconds: number) {
    if (intervalSeconds > 0) {
      ...
      console.log(
        `Intervalo de spawn de comida actualizado: ${intervalSeconds}s (${this.ticksPerFoodSpawn} ticks)`,
      );
    } else {
  ...
  private update() {
    ...
    this.world.individuals.forEach((individual) => {
      ...
      const energyCost =
        individual.currentState === 'hunting' || individual.currentState === 'fleeing'
          ? 0.03
    ...
    if (this.ticksPerFoodSpawn > 0 && this.world.foodSources.length < 2000) {
      ...
      if (this.foodSpawnTickCounter >= adjustedTicksPerSpawn) {
        ...
        console.log(
          `Food spawned! Interval: ${((adjustedTicksPerSpawn * this.world.cycleDurationMs) / 1000).toFixed(2)}s (multiplier: ${spawnIntervalMultiplier}x)`,
        );
      }
    }
  ...
  private render() {
    ...
    const averageHunger =
      this.world.individuals.reduce((sum, i) => sum + i.hunger, 0) / Math.max(1, population);
    ...
    const hungerColor =
      hungerPercent > 0.7 ? '#FF3333' : hungerPercent > 0.4 ? '#FFA500' : '#00FF00';
  ...
  private evaluateState(individual: Individual) {
    ...
    if (individual.hunger > 60) {
      ...
      individual.age > 18 &&
      individual.energy > 60 &&
      individual.cooldownUntil < this.world.tick
    ) {
  ...
  private moveIndividual(individual: Individual) {
    ...
    if (individual.currentState === 'fleeing') {
      ...
      const threats = this.world.individuals.filter(
        (other) =>
          other.isAlive &&
  ...
  private handleEating(individual: Individual) {
    ...
    const targetFood = this.world.foodSources.find(
      (f) => f.id === individual.targetId && !f.isConsumed,
    );
  ...
  private handleHunting(individual: Individual) {
    ...
    if (distance < individual.size + prey.size) {
      ...
      if (hunterPower > preyPower * 1.1) {
        // RELAJADO: Solo necesita 10% de superioridad (antes 20%)
  ...
  private reproduce(parent1: Individual, parent2: Individual) {
    ...
    const creatureType =
      newIndividual.race === 'circle'
        ? 'Beast'
        : newIndividual.race === 'square'
          ? 'Titan'
          : newIndividual.race === 'triangle'
            ? 'Predator'
            : 'Creature';
